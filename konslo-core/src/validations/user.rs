...
pub fn validate_username(username: &str) -> Result<(), AppError> {
    ...
    if length < 4 {
        Err(AppError::Validation("username must be at least 4 characters".into()))
    } else if length > 20 {
        Err(AppError::Validation("username must be at most 20 characters".into()))
    } else if !username.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-') {
        Err(AppError::Validation("username may only contain letters, digits, underscores, and hyphens".into()))
...
pub fn validate_password(password: &str) -> Result<(), AppError> {
    ...
    match password.len() {
        0..=7 => Err(AppError::Validation("password must be at least 8 characters".into())),
        73.. => Err(AppError::Validation("password must be at most 72 characters".into())),
        _ => Ok(()),
    }
}
...
mod tests {
    ...
    fn test_validate_username_3_chars_returns_validation_error() {
        assert!(matches!(validate_username("bob").unwrap_err(), AppError::Validation(_)));
    }
    ...
    fn test_validate_username_21_chars_returns_validation_error() {
        assert!(matches!(validate_username(&"x".repeat(21)).unwrap_err(), AppError::Validation(_)));
    }

    #[test]
    fn test_validate_username_invalid_chars_returns_validation_error() {
        assert!(matches!(validate_username("!Tocqueville!").unwrap_err(), AppError::Validation(_)));
    }

    #[test]
    fn test_validate_username_demo_returns_validation_error() {
        assert!(matches!(validate_username("demo").unwrap_err(), AppError::Validation(_)));
    }
    ...
    fn test_validate_password_7_chars_returns_validation_error() {
        assert!(matches!(validate_password(&"x".repeat(7)).unwrap_err(), AppError::Validation(_)));
    }
    ...
    fn test_validate_password_73_chars_returns_validation_error() {
        assert!(matches!(validate_password(&"x".repeat(73)).unwrap_err(), AppError::Validation(_)));
    }
}
